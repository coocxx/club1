...
<body>
	...
	<div
		...
		<form class="layui-form" method="post" name="form1"
			...
			action="${pageContext.request.contextPath }/admin/selAppEvents">
	...
	<c:if test="${ curPage.data!=null }">
							...
							<td>
								<div>
									<a class="layui-btn layui-btn-radius layui-btn-normal"
										href="${pageContext.request.contextPath }/admin/selAppEventById?id=${i.id }">详情</a>
								</div> <c:if test="${i.status==2 }">
									<div>
										<a class="layui-btn layui-btn-radius"
											href="${pageContext.request.contextPath }/admin/updAppEventStatus?id=${i.id }&student.id=${i.student.id }&club.id=${i.id }&status=1"
											onclick="return updConfirm()">通过</a>
									</div>
								</c:if> <c:if test="${i.status==2 }">
									<div>
										<a class="layui-btn layui-btn-radius layui-btn-warm"
											href="${pageContext.request.contextPath }/admin/updAppEventStatusR?id=${i.id }&student.id=${i.student.id }&status=0"
		...
		<div style="margin: 10px 0px 10px 380px">
			...
			<c:if test="${curPage.curPage!=1}">
				<a class="layui-btn"
					href="${pageContext.request.contextPath }/admin/selAppEventByPage?curPage=1">首页</a>
			</c:if>
			<!-- 上一页 -->
			<c:if test="${curPage.curPage<=1}">
				<a class="layui-btn layui-btn-primary"
			...
			<c:if
				...
				<a class="layui-btn"
					href="${pageContext.request.contextPath }/admin/selAppEventByPage?curPage=${curPage.curPage-1 }"><i
			...
			<c:if test="${curPage.curPage!=curPage.totalPage}">
				<a class="layui-btn"
					href="${pageContext.request.contextPath }/admin/selAppEventByPage?curPage=${curPage.totalPage }">尾页</a>
			</c:if>
		</div>
	</c:if>
	<script>
