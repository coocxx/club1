...
<body>
    ...
    <!-- 按条件查询 -->
    <div style="margin:0px auto;background-color:rgba(57, 61, 73, 0.1);height:200px;">
       
        <form class="layui-form" method="post" name="form1" style="margin-left:70px;padding-top: 20px;"
            action="${pageContext.request.contextPath }/admin/selClubApplies">
            <div class="layui-form-item">
                ...
                <div class="layui-inline">
                    <div class="layui-input-block" style="width: 170px;margin-top:5px">
    ...
    <c:if test="${ curPage.data!=null }">
                ...
                <tbody>
                    <c:forEach items="${curPage.data }" var="i">
                        <tr>
                            ...
                            <td><c:if test="${i.adminId!=0 }">${i.adminId }</c:if></td>
                            <td>${i.name }</td>
                            <td>${i.reason }</td>
                            <td><c:if test="${i.status==0 }">拒绝</c:if> <c:if
                                    test="${i.status==1 }">通过</c:if>
                                    <c:if test="${i.status==2 }">待审核</c:if></td>
                            
                            <td>${i.time }</td>
                            <td>
                               <div> <a class="layui-btn layui-btn-radius layui-btn-normal"
                                href="${pageContext.request.contextPath }/admin/selClubApplyById?id=${i.id }" >详情</a>
                                </div><div>
                                <c:if test="${i.status==2 }"><div><a class="layui-btn layui-btn-radius"
                                href="${pageContext.request.contextPath }/admin/updClubApplyStatus?id=${i.id }&studentId=${i.studentId }&name=${i.name }&clubTypeId=${i.clubTypeId }&status=1" onclick="return updConfirm()">通过</a>
                                </div></c:if>
                                
                                <c:if test="${i.status==2 }"><div><a class="layui-btn layui-btn-radius layui-btn-warm"
                                href="${pageContext.request.contextPath }/admin/updClubApplyStatusR?id=${i.id }&studentId=${i.studentId }&status=0" onclick="return rejConfirm()">拒绝</a>
                                </c:if></div>
                              <c:if test="${i.status==1 || i.status==0}"><div><a class="layui-btn layui-btn-radius layui-btn-warm"
                                href="${pageContext.request.contextPath }/admin/delClubApply?id=${i.id }"
        ...
        <div style="margin: 10px 0px 10px 380px">
            ...
            <c:if test="${curPage.curPage!=1}">
                <a class="layui-btn"
                    href="${pageContext.request.contextPath }/admin/selClubApplyByPage?curPage=1">首页</a>
            </c:if>
            <!-- 上一页 -->
            <c:if test="${curPage.curPage<=1}">
                <a class="layui-btn layui-btn-primary"
            ...
            <c:if
                ...
                <a class="layui-btn"
                    href="${pageContext.request.contextPath }/admin/selClubApplyByPage?curPage=${curPage.curPage-1 }"><i class="layui-icon layui-icon-left"></i></a>
            </c:if>
            <!-- 下一页 -->
            ...
            <c:if
                test="${(curPage.curPage>=1) && (curPage.curPage<curPage.totalPage)}">
                <a class="layui-btn"
                    href="${pageContext.request.contextPath }/admin/selClubApplyByPage?curPage=${curPage.curPage+1 }"><i class="layui-icon">&#xe602;</i></a>
            </c:if>
            <!-- 尾页 -->
            <c:if test="${curPage.curPage==curPage.totalPage}">
                <a
            ...
            <c:if test="${curPage.curPage!=curPage.totalPage}">
                <a class="layui-btn"
                    href="${pageContext.request.contextPath }/admin/selClubApplyByPage?curPage=${curPage.totalPage }">尾页</a>
            </c:if>
        </div>
    </c:if>
    <script>
        ...
        layui.use([ 'element', 'form', 'laydate' ], function() {
            ...
            form.on('submit(setId)', function(data) {
                ...
                if (document.form1.adminId.value == ''
                        || document.form1.adminId.value == 0) {
                    document.form1.adminId.value = 0;
                } 
                return true;
            });
