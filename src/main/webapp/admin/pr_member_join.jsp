...
<body>
    ...
    <div
        ...
        <form class="layui-form" method="post" name="form1"
            ...
            action="${pageContext.request.contextPath }/admin/selMemberJoins?active=pr">
    ...
    <c:if test="${ curPage.data!=null && curPage.data.size()!=0 }">
                ...
                <tbody>
                    <c:forEach items="${curPage.data }" var="i">
                        <tr>
                            ...
                            <td>
                              <div> <a class="layui-btn layui-btn-normal"
                                href="${pageContext.request.contextPath }/admin/selMemberJoinById?active=pr&id=${i.id }" >详情</a>
                                </div><div>
                                <c:if test="${i.status==2 }"><div><a class="layui-btn "
                                href="${pageContext.request.contextPath }/admin/updMemberJoinStatus?active=pr&id=${i.id }&studentId=${i.student.id }&clubId=${president.club.id }&status=1" onclick="return updConfirm()">通过</a>
                                </div></c:if>
                                
                                <c:if test="${i.status==2 }"><div><a class="layui-btn layui-btn-warm"
                                href="${pageContext.request.contextPath }/admin/updMemberJoinStatusR?active=pr&id=${i.id }&studentId=${i.student.id }&clubId=${president.club.id }&status=0" onclick="return rejConfirm()">拒绝</a>
                                </c:if></div>
                              <c:if test="${i.status==1 || i.status==0}"><div><a class="layui-btn layui-btn-warm"
                                href="${pageContext.request.contextPath }/admin/delMemberJoin?active=pr&id=${i.id }"
        ...
        <div style="margin: 10px 0px 10px 380px">
            ...
            <c:if test="${curPage.curPage!=1}">
                <a class="layui-btn"
                    href="${pageContext.request.contextPath }/admin/selMemberJoinByPage?active=pr&curPage=1">首页</a>
            </c:if>
            <!-- 上一页 -->
            <c:if test="${curPage.curPage<=1}">
                <a class="layui-btn layui-btn-primary"
            ...
            <c:if
                ...
                <a class="layui-btn"
                    href="${pageContext.request.contextPath }/admin/selMemberJoinByPage?active=pr&curPage=${curPage.curPage-1 }"><i
            ...
            <c:if test="${curPage.curPage!=curPage.totalPage}">
                <a class="layui-btn"
                    href="${pageContext.request.contextPath }/admin/selMemberJoinByPage?active=pr&curPage=${curPage.totalPage }">尾页</a>
            </c:if>
        </div>
    </c:if>
    <script>
