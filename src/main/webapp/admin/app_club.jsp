...
<body>
    ...
    <div style="margin:0px auto;background-color:rgba(57, 61, 73, 0.1);height:130px;">
        ...
        <form class="layui-form" method="post" name="form1" style="margin-left:70px;padding-top: 20px;"
            action="${pageContext.request.contextPath }/admin/selAppClubs">
    ...
    <c:if test="${ curPage.data!=null}">
                        ...
                        <tr>
                            ...
                            <td>
                               <div> <a class="layui-btn layui-btn-radius layui-btn-normal"
                                href="${pageContext.request.contextPath }/admin/selAppClubById?id=${i.id }" >详情</a>
                                </div><div>
                                <c:if test="${i.status==2 }"><div><a class="layui-btn layui-btn-radius"
                                href="${pageContext.request.contextPath }/admin/updAppClubStatus?id=${i.id }&studentId=${i.studentId }&name=${i.name }&clubTypeId=${i.clubTypeId }&status=1" onclick="return updConfirm()">通过</a>
                                </div></c:if>
                                
                                <c:if test="${i.status==2 }"><div><a class="layui-btn layui-btn-radius layui-btn-warm"
                                href="${pageContext.request.contextPath }/admin/updAppClubStatusR?id=${i.id }&status=0&studentId=${i.studentId }" onclick="return rejConfirm()">拒绝</a>
                                </div></c:if>
                             </td> 
                        </tr>
                    </c:forEach>
        ...
        <div style="margin: 10px 0px 10px 380px">
            ...
            <c:if test="${curPage.curPage!=1}">
                <a class="layui-btn"
                    href="${pageContext.request.contextPath }/admin/selAppClubByPage?curPage=1">首页</a>
            </c:if>
            <!-- 上一页 -->
            <c:if test="${curPage.curPage<=1}">
                <a class="layui-btn layui-btn-primary"
            ...
            <c:if
                ...
                <a class="layui-btn"
                    href="${pageContext.request.contextPath }/admin/selAppClubByPage?curPage=${curPage.curPage-1 }"><i class="layui-icon layui-icon-left"></i></a>
            </c:if>
            <!-- 下一页 -->
            ...
            <c:if
                test="${(curPage.curPage>=1) && (curPage.curPage<curPage.totalPage)}">
                <a class="layui-btn"
                    href="${pageContext.request.contextPath }/admin/selAppClubByPage?curPage=${curPage.curPage+1 }"><i class="layui-icon">&#xe602;</i></a>
            </c:if>
            <!-- 尾页 -->
            <c:if test="${curPage.curPage==curPage.totalPage}">
                <a
            ...
            <c:if test="${curPage.curPage!=curPage.totalPage}">
                <a class="layui-btn"
                    href="${pageContext.request.contextPath }/admin/selAppClubByPage?curPage=${curPage.totalPage }">尾页</a>
            </c:if>
        </div>
    </c:if>
    <script>
