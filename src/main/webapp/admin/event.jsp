...
<body>
	...
	<div
		...
		<form class="layui-form" method="post" name="form1"
			...
			action="${pageContext.request.contextPath }/admin/selEvents">
			<div class="layui-form-item">
				...
				<div class="layui-inline">
					...
					<div class="layui-input-inline" style="width: 150px;">
						<input type="text" name="club.name" autocomplete="off" value=""
							class="layui-input">
					</div>
				</div>
				<div class="layui-inline">
				...
				<div class="layui-inline">
					...
					<div class="" style="display: inline; width: 150px;">
						<input type="radio" name="status" value="0" title="待审核"> <input
							type="radio" name="status" value="1" title="审核通过"> <input
							type="radio" name="status" value="2" title="未通过"> <input
	...
	<c:if test="${ curPage.data!=null }">
							...
							<td>
								<div>
									<a class="layui-btn layui-btn-radius layui-btn-normal"
										href="${pageContext.request.contextPath }/admin/preUpdEvent?id=${i.id }">编辑</a>
								</div> <c:if test="${i.status==2 }">
									<div>
										<a class="layui-btn layui-btn-radius"
											href="${pageContext.request.contextPath }/admin/updEventStatus?id=${i.id }&student.id=${i.studentId }&club.id=${i.id }&status=1"
											onclick="return updConfirm()">通过</a>
									</div>
								</c:if> <c:if test="${i.status==2 }">
									<div>
										<a class="layui-btn layui-btn-radius layui-btn-warm"
											href="${pageContext.request.contextPath }/admin/updEventStatusR?id=${i.id }&student.id=${i.studentId }&status=0"
											onclick="return rejConfirm()">拒绝</a>
									</div>
								</c:if> <c:if test="${i.status==1 || i.status==0}">
									<div>
										<a class="layui-btn layui-btn-radius layui-btn-warm"
											href="${pageContext.request.contextPath }/admin/delEvent?id=${i.id }"
		...
		<div style="margin: 10px 0px 10px 380px">
			...
			<c:if test="${curPage.curPage!=1}">
				<a class="layui-btn"
					href="${pageContext.request.contextPath }/admin/selEventByPage?curPage=1">首页</a>
			</c:if>
			<!-- 上一页 -->
			<c:if test="${curPage.curPage<=1}">
				<a class="layui-btn layui-btn-primary"
			...
			<c:if
				...
				<a class="layui-btn"
					href="${pageContext.request.contextPath }/admin/selEventByPage?curPage=${curPage.curPage-1 }"><i
			...
			<c:if test="${curPage.curPage!=curPage.totalPage}">
				<a class="layui-btn"
					href="${pageContext.request.contextPath }/admin/selEventByPage?curPage=${curPage.totalPage }">尾页</a>
			</c:if>
		</div>
	</c:if>
	<script>
