...
<body>
    ...
    <div
        ...
        <form class="layui-form" method="post" name="form1"
            ...
            action="${pageContext.request.contextPath }/admin/selClubMembers?active=pr">
                ...
                <div class="layui-inline">
                    ...
                    <div class="" style="display: inline; width: 150px;">
                        <input type="radio" name="status" value="0" title="成员"> <input
                            ...
                            type="radio" name="status" value="2" title="往届会长" >
                            <input type="radio" name="status" value="3" title="往届成员" >
                            <input type="radio" name="status" value="4" title="全部" checked> 
                    </div>
                </div>
    ...
    <c:if test="${ curPage.data!=null && curPage.data.size()!=null }">
                ...
                <tbody>
                    <c:forEach items="${curPage.data }" var="i">
                        <tr>
                            ...
                            <td><c:if test="${i.status==0 }">成员</c:if> 
                            <c:if test="${i.status==1 }">会长</c:if>
                                <c:if test="${i.status==2 }">往届会长</c:if> 
                                <c:if test="${i.status==3 }">往届成员</c:if></td>
                            <td>${i.time }</td>
                            <td><div><a class="layui-btn layui-btn-radius layui-btn-normal"
                                href="${pageContext.request.contextPath }/admin/preUpdClubMember?active=pr&id=${i.id }"><i class="layui-icon">&#xe642;编辑</i></a>
                                    </div>
                                <div><a class="layui-btn layui-btn-radius"
                                href="${pageContext.request.contextPath }/admin/delClubMember?active=pr&id=${i.id }"
                                ...
                                <c:if test="${i.status==0 || i.status==1}">
									<div>
										<a class="layui-btn layui-btn-radius layui-btn-warm"
											href="${pageContext.request.contextPath }/admin/quitClubMember?active=pr&id=${i.id }&clubId=${president.club.id}&studentId=${i.studentId}"
											onclick="return loConfirm()">退出社团</a>
									</div>
								</c:if>
                              </td>
                        </tr>
        ...
        <div style="margin: 10px 0px 10px 380px">
            ...
            <c:if test="${curPage.curPage!=1}">
                <a class="layui-btn"
                    href="${pageContext.request.contextPath }/admin/selClubMemberByPage?active=pr&curPage=1">首页</a>
            </c:if>
            <!-- 上一页 -->
            <c:if test="${curPage.curPage<=1}">
                <a class="layui-btn layui-btn-primary"
            ...
            <c:if
                ...
                <a class="layui-btn"
                    href="${pageContext.request.contextPath }/admin/selClubMemberByPage?active=pr&curPage=${curPage.curPage-1 }"><i
            ...
            <c:if test="${curPage.curPage!=curPage.totalPage}">
                <a class="layui-btn"
                    href="${pageContext.request.contextPath }/admin/selClubMemberByPage?active=pr&curPage=${curPage.totalPage }">尾页</a>
            </c:if>
        </div>
    </c:if>
    <script>
